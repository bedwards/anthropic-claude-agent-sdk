"""
GitHub operations for worker agents.

Handles PRs, reviews, issues, and CI checks.
This is a shared component used by all worker types.
"""
...
class GitHubOperations:
    ...
    def __init__(
        ...
        repo_owner: str,
        repo_name: str,
        status_manager: "BaseStatusManager",  # type: ignore[type-arg]
    ) -> None:
    ...
    async def get_existing_pr(self, branch: str) -> dict[str, int | str] | None:
        ...
        pulls = self.repo.get_pulls(state="open", head=f"{self.repo_owner}:{branch}")
        for pr in pulls:
            return {
                "number": pr.number,
                "url": pr.html_url,
            ...
            result.append(
                PRReview(
                    ...
                    if review_comments
                        ...
                        ReviewComment(
                            path="", line=0, body=comment.body, is_blocking=requests_changes
                        )
                    ],
                )
            )
    ...
    async def get_pr_check_status(self, pr_number: int) -> CIStatus:
        ...
        if not check_runs and not statuses:
            ...
            self.status_manager.log(LogLevel.INFO, "No CI checks configured, treating as success")
...
_Created automatically by worker agent from non-blocking review comment_""",
        ...
        self.status_manager.log(
            LogLevel.INFO, f"Created issue #{issue.number} for non-blocking feedback"
        )
        return issue.number
